...
interface Ticket {
  ...
  site_name?: string;
  site_code?: string;
  site?: { id: number; site_code?: string; site_name?: string; connection_status?: string } | null;
...
const TicketDetailView: React.FC<TicketDetailViewProps> = ({ ticket, timeline, mode }) => {
  const getStatusBadge = (status: string) => {
    ...
    const configs: Record<string, { icon: JSX.Element; bg: string; text: string; color: string }> = {
      ...
      OPEN: { icon: <AlertCircle className="h-4 w-4" />, bg: 'bg-blue-100', text: 'In Progress', color: 'text-[var(--primary-hover)]' },
      IN_PROGRESS: { icon: <AlertCircle className="h-4 w-4" />, bg: 'bg-blue-100', text: 'In Progress', color: 'text-[var(--primary-hover)]' },
        ...
        <CardHeader className="pb-4">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              {!!ticket.tags?.length && (
                <div className="mt-3">
                  <TicketTagBadgesRow tags={ticket.tags} max={8} />
                </div>
              )}
            </div>
                ...
                <div key={step} className="flex items-center flex-1">
                  <div className="flex flex-col items-center">
                    ...
                    <span className={`mt-3 text-sm font-medium
                      ${isCompleted ? 'text-green-700' : isActive ? 'text-[var(--primary)]' : 'text-gray-500'}
                    `}>
                      {step}
                    </span>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="font-semibold flex items-center gap-2">
                  ...
                  {ticket.site?.site_name || ticket.site_name || ticket.project_name || '—'}
                  {(ticket.site?.site_code || ticket.site_code) && (
                    <span className="rounded bg-[var(--accent-green-light)] px-2 py-0.5 font-mono text-xs text-[var(--primary)]">
                      {ticket.site?.site_code || ticket.site_code}
                    </span>
                  )}
                </p>
              ...
              <div className="space-y-8">
                {visibleTimeline.map((entry, idx) => (
                  <div key={idx} className="relative flex gap-6">
                    <div className="flex-shrink-0 w-12 h-12 rounded-full bg-[var(--primary)] text-white flex items-center justify-center shadow-md z-10">
                      ...
                      <div className="bg-white border rounded-xl p-6 shadow-[var(--shadow-md)]">
                        <div className="flex items-start justify-between mb-3">
                          ...
                          {entry.visibility === 'internal' && mode === 'staff' && (
                            <span className="px-3 py-1 bg-[var(--accent-green-light)] text-[var(--primary)] text-xs font-bold rounded-full">
                              Internal Note
                            </span>
                          )}
                        </div>

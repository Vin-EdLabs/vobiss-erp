...
import {
  Archive as ArchiveIcon, Search, Plus, Folder, FolderOpen, Upload, Download, Eye,
  Trash2, Pencil, ChevronLeft, ChevronRight, FolderX, Lock, ShieldCheck,
} from 'lucide-react';
...
import { fileIconFor, formatFileSize, formatUnitLabel } from '@/components/archive/shared';
...
export default function Archive() {
  ...
  const doUpload = async (fileList: FileList | File[]) => {
    ...
    try {
      setUploading(true);
      await uploadArchiveFiles(selectedFolderId, arr);
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h1 className="mt-1 flex items-center gap-2 text-3xl font-bold text-[var(--text-primary)]"><ArchiveIcon className="h-7 w-7" /> Archive</h1>
          <p className="mt-1 text-sm text-[var(--text-secondary)]">Historical Excel, PDF, KMZ and other files, kept in named folders — global, by department, or private to you.</p>
        </div>
          ...
          {!selectedFolder ? (
            ...
            <>
              <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-xs text-[var(--text-muted)]">Archive / {selectedFolder.scope === 'global' ? 'Global' : selectedFolder.scope === 'private' ? 'Private' : formatUnitLabel(selectedFolder.unit_slug)}</p>
              ...
              >
                {filesLoading ? (
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {files.map((f) => (
                      <FileCard key={f.id} file={f} onPreview={() => setPreviewFile(f)} onDownload={() => downloadArchiveFile(f.id, f.display_name)} onDelete={() => setFileToDelete(f)} canDelete={f.uploaded_by === user?.id} />
                    ))}
                  </div>
                )}
              </div>
...
function FileCard({ file, onPreview, onDownload, onDelete, canDelete }: {
  file: ArchiveFile; onPreview: () => void; onDownload: () => void; onDelete: () => void; canDelete: boolean;
}) {
  ...
  return (
    <div className="group rounded-xl border border-[var(--border)] p-3 transition hover:border-[var(--primary)] hover:shadow-[var(--shadow-sm)]">
      <div className="mb-2 flex items-start gap-2">
        <Icon className="h-8 w-8 shrink-0 text-[var(--primary)]" />
        <div className="min-w-0 flex-1">
      ...
      <div className="flex items-center gap-1.5 opacity-0 transition group-hover:opacity-100">
        ...
        {canDelete && <Button type="button" size="icon" variant="ghost" className="h-7 w-7 text-[var(--danger-text)]" onClick={onDelete}><Trash2 className="h-3.5 w-3.5" /></Button>}
      </div>
    </div>
  );
}
